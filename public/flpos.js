(() => {
  ...
  function priceForSupplier(product) {
  ...
  function resolveLinePrice(product) {
    ...
    return priceForSupplier(product);
  }
  ...
  function setSupplierCreateVisible(visible) {
  ...
  function renderSupplierLetterBar() {
  ...
  function setActiveSupplierLetter(letter) {
    if (!customerLetterBar) return;
  ...
  function syncSupplierLetterFromInput() {
    if (!customerSearch) return;
    const value = (customerSearch.value || "").trim();
    if (value.length === 1 && /^[a-z]$/i.test(value)) {
      setActiveSupplierLetter(value.toUpperCase());
    } else {
      setActiveSupplierLetter("");
    }
  }
  ...
  function renderProductsTable() {
    ...
    productsBody.innerHTML = filtered
      .map((p) => {
        ...
        const price = priceForSupplier(p);
  ...
  function renderSupplierChips() {
    ...
    customerChips.innerHTML = `
      <span class="flocs-chip">Supplier: ${c.name}</span>
  ...
  function renderAddressSelect() {
    ...
    if (!addrs.length) {
      addrSelect.innerHTML =
        `<option value="">No addresses on supplier</option>`;
      addrSelect.disabled = true;
      addrPreview.hidden = true;
      return;
    }
...
${state.customer.email || ""}${
    ...
    invoice.innerHTML = `
      <div class="flocs-invoiceHeader">
        <div>
          ...
          <div class="flocs-invoiceSub">Purchase order preview (FLPOS)</div>
        </div>
      ...
      <div class="flocs-invoiceNote">
        ${pricingNote}${overrideNote}. Final pricing and tax are still controlled in Shopify. FLPOS only seeds the purchase order draft.
      </div>
    `;
  }
  ...
  function validate() {
    ...
    if (currentDelivery() === "ship") {
      ...
      if (!state.shippingQuote) {
        errs.push("Calculate shipping (SWE quote) before locking the purchase order.");
      }
    }
    ...
    if (previewTag) {
      if (!state.customer) {
        previewTag.textContent = "Waiting for supplier…";
        ...
        previewTag.textContent = "Incomplete purchase order";
      }
    }
  ...
  async function requestShippingQuote() {
    ...
    const details = {
      ...
      notes: `FLPOS purchase order pre-quote`,
  ...
  // ===== Shopify calls =====
  const searchSuppliersDebounced = debounce(searchSuppliersNow, 320);
  const searchProductsDebounced = debounce(searchProductsNow, 320);

  async function searchSuppliersNow() {
    ...
    try {
      ...
      if (!list.length) {
        customerResults.innerHTML =
          `<div class="flocs-customerEmpty">No suppliers found.</div>`;
      ...
      customerStatus.textContent =
        "Click a row to select supplier.";
    } catch (e) {
      ...
      customerStatus.textContent = "Error searching suppliers.";
    }
  }
  ...
  async function searchProductsNow() {
    ...
    try {
      ...
      productStatus.textContent = "Click add to include in the purchase order.";
    } catch (e) {
  ...
  async function createSupplier() {
    ...
    if (customerCreateStatus) {
      customerCreateStatus.textContent = "Creating supplier…";
    }
    ...
    try {
      ...
      if (!res.ok || !data.ok) {
        console.error("Supplier create error:", data);
        showToast("Supplier create failed.", "err");
        if (customerCreateStatus) {
          customerCreateStatus.textContent =
            data?.body?.errors || data?.message || "Supplier create failed.";
        }
        return;
      }

      if (!data.customer) {
        showToast("Supplier created, but no data returned.", "err");
        return;
      }

      applySelectedSupplier(data.customer);
      if (customerCreateStatus) {
        customerCreateStatus.textContent =
          `Created: ${data.customer.name}`;
      }
      showToast("Supplier created.", "ok");
      resetSupplierForm();
  ...
  function resetSupplierForm() {
    ...
    if (customerCreateStatus) {
      customerCreateStatus.textContent =
        "Add a supplier if search returns nothing.";
    }
  }

  function applySelectedSupplier(c) {
    state.customer = c;
  ...
  async function createDraftOrder() {
    if (state.errors.length) {
      showToast("Fix errors before locking purchase order.", "err");
      return;
    }
    ...
    confirmBtn.textContent = "Creating purchase order draft…";
    ...
    try {
      ...
      if (!resp.ok || !data.ok) {
        console.error("Purchase order draft create error:", text);
        showToast(
          "Purchase order draft failed: " +
      ...
      const d = data.draftPurchase order || {};
      let link = "";
      if (d.adminUrl) {
        link = `\nOpen: ${d.adminUrl}`;
      ...
      showToast(
        `Purchase order draft ${d.name || d.id} created.${link ? " See console for link." : ""}`,
        "ok"
      );
      if (d.adminUrl) {
        console.log("Purchase order draft admin URL:", d.adminUrl);
      }
      ...
      console.error("Purchase order draft create exception:", e);
      showToast("Purchase order draft error: " + String(e?.message || e), "err");
    } finally {
      state.isSubmitting = false;
      confirmBtn.textContent = "Lock purchase order & create Shopify draft";
      validate();
    }
  }
  ...
  async function createOrderNow() {
    if (state.errors.length) {
      showToast("Fix errors before creating purchase order.", "err");
      return;
    }
    ...
    try {
      ...
      if (!resp.ok || !data.ok) {
        console.error("Purchase order create error:", text);
        showToast(
      ...
      showToast(
        `Purchase order ${data.order?.name || data.order?.id || ""} created.`,
        "ok"
      );
      resetForm();
    } catch (e) {
      console.error("Purchase order create exception:", e);
      showToast("Purchase order create error: " + String(e?.message || e), "err");
    } finally {
      state.isSubmitting = false;
      if (createOrderBtn) {
  ...
  function resetForm(options = {}) {
    const { keepDraftPurchase order = false } = options;
    state.customer = null;
    state.po = "";
    state.delivery = "ship";
  ...
  async function convertDraftToOrder() {
    if (!state.lastDraftOrderId) {
      showToast("No purchase order draft to convert yet.", "err");
      return;
    }
    ...
    try {
      ...
      if (!resp.ok || !data.ok) {
        console.error("Purchase order draft complete error:", text);
        showToast(
          "Purchase order draft convert failed: " +
            (data?.body?.errors || resp.statusText || "unknown"),
          "err"
        );
        return;
      ...
      showToast(`Draft converted to purchase order ${data.order?.name || ""}`.trim(), "ok");
      state.lastDraftOrderId = null;
      renderConvertButton();
    } catch (e) {
      console.error("Purchase order draft complete exception:", e);
  ...
  function initEvents() {
    if (customerSearch) {
      customerSearch.addEventListener("input", () => {
        syncSupplierLetterFromInput();
        searchSuppliersDebounced();
      });
    }
    ...
    if (customerLetterBar) {
      customerLetterBar.addEventListener("click", (e) => {
        ...
        setActiveSupplierLetter(letter);
        searchSuppliersNow();
      });
    }
    ...
    if (customerCreateToggle) {
      customerCreateToggle.addEventListener("click", () => {
        ...
        setSupplierCreateVisible(isHidden);
      });
    }
