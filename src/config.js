...
export const config = {
  ...
  DELIVERY_CODE_SECRET: process.env.DELIVERY_CODE_SECRET,
  SHOPIFY_API_VERSION: process.env.SHOPIFY_API_VERSION || "2025-10",
  SHOPIFY_FLOW_TAG: process.env.SHOPIFY_FLOW_TAG || "dispatch_flow",
  SHOPIFY_TIMEOUT_MS: numberOrDefault(process.env.SHOPIFY_TIMEOUT_MS, 10000, { min: 1 }),
  SHOPIFY_THROTTLE_MAX_CONCURRENCY: numberOrDefault(process.env.SHOPIFY_THROTTLE_MAX_CONCURRENCY, 4, {
    min: 1
  }),
  SHOPIFY_THROTTLE_BASE_DELAY_MS: numberOrDefault(process.env.SHOPIFY_THROTTLE_BASE_DELAY_MS, 250, {
    min: 0
  }),
  SHOPIFY_THROTTLE_MAX_DELAY_MS: numberOrDefault(process.env.SHOPIFY_THROTTLE_MAX_DELAY_MS, 5000, {
    min: 1
  }),
  SHOPIFY_THROTTLE_CALL_LIMIT_RATIO: numberOrDefault(
  ...
  SMTP_PORT: numberOrDefault(process.env.SMTP_PORT, 587, { min: 1 }),
  ...
  SHOPIFY_CUSTOMER_ACCOUNT_CLIENT_SECRET: process.env.SHOPIFY_CUSTOMER_ACCOUNT_CLIENT_SECRET || "",
  SHOPIFY_CUSTOMER_ACCOUNT_SCOPES:
    process.env.SHOPIFY_CUSTOMER_ACCOUNT_SCOPES || "openid email customer-account-api:full",
  SHOPIFY_CUSTOMER_ACCOUNT_REDIRECT_URI: process.env.SHOPIFY_CUSTOMER_ACCOUNT_REDIRECT_URI || "",
  SHOPIFY_CUSTOMER_ACCOUNT_POST_LOGOUT_REDIRECT_URI:
    process.env.SHOPIFY_CUSTOMER_ACCOUNT_POST_LOGOUT_REDIRECT_URI || "",
  SHOPIFY_CUSTOMER_ACCOUNT_COOKIE_SECURE: process.env.SHOPIFY_CUSTOMER_ACCOUNT_COOKIE_SECURE || "",
  SHOPIFY_CUSTOMER_ACCOUNT_SESSION_TTL_MS: numberOrDefault(
    process.env.SHOPIFY_CUSTOMER_ACCOUNT_SESSION_TTL_MS,
    8 * 60 * 60 * 1000,
    {
      min: 60000
    }
