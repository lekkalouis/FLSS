...
router.get("/shopify/orders/open", async (req, res) => {
  try {
    ...
    const stripOrigin = (fullUrl) => {
      const httpPrefix = "http://";
      const httpsPrefix = "https://";
      let working = fullUrl;
      if (working.startsWith(httpPrefix)) working = working.slice(httpPrefix.length);
      else if (working.startsWith(httpsPrefix)) working = working.slice(httpsPrefix.length);
      const slashIndex = working.indexOf("/");
      return slashIndex === -1 ? "" : working.slice(slashIndex);
    };
